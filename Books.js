...
import { Button, FlatList, Modal, Box, Center, NativeBaseProvider, Image, Text } from "native-base";
...
},
{
  ...
  return (
    <FlatList
      data={data}
      renderItem={({ item }) => (
        <Box px={2} py={15} rounded="md" my={2} bg="primary.300" height="auto">
          <Text style={{fontWeight: 700, paddingBottom: 10}}>"{item.title}"</Text>
          <Text style={{fontSize: 14, fontWeight: 600, paddingBottom: 10}}> by {item.authors}</Text>
          <Text style={{fontSize: 14, paddingBottom: 10}}>Category(-ies): {item.categories}</Text>
          <Text style={{fontSize: 14, textAlign: "left"}}>{item.longDescription}</Text>
        </Box>
      )}
